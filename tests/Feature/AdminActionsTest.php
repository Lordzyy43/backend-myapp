...
class AdminActionsTest extends TestCase
{
  ...
  protected function setUp(): void
  {
    ...
    $this->venue = Venue::create([
      ...
      'city' => 'Bandung',
      'phone' => '08987654321',
      'email' => 'admin-venue-' . rand(1, 99) . '@test.com',
    ...
    // 4. CREATE TIME SLOTS
    TimeSlot::firstOrCreate(['start_time' => '10:00', 'end_time' => '11:00', 'order_index' => 1, 'is_active' => true]);
  ...
  {
    $response = $this->actingAs($this->admin, 'sanctum')
      ->postJson("/api/v1/admin/bookings/{$this->booking->id}/approve");
  ...
  {
    $response = $this->actingAs($this->admin, 'sanctum')
      ->postJson("/api/v1/admin/bookings/{$this->booking->id}/reject");
  ...
  public function admin_can_finish_booking()
  {
    ...
    $response = $this->actingAs($this->admin, 'sanctum')
      ->postJson("/api/v1/admin/bookings/{$this->booking->id}/finish");
  ...
  {
    $response = $this->actingAs($this->admin, 'sanctum')
      ->postJson("/api/v1/admin/payments/{$this->payment->id}/approve");
  ...
  {
    $response = $this->actingAs($this->admin, 'sanctum')
      ->postJson("/api/v1/admin/payments/{$this->payment->id}/reject");

    $response->assertStatus(200)
      ->assertJson(['success' => true]);

    $this->payment->refresh();
    $this->assertEquals('cancelled', $this->payment->status->status_name);
  }
  ...
  {
    $response = $this->actingAs($this->admin, 'sanctum')
      ->getJson('/api/v1/admin/bookings/report');
